...
if submit_btn:
    if not api_key_input:
        ...
        try:
            ...
            JSON 형식으로 응답해야 하며, 각 항목은 다음 필드를 가져야 해:
            majorName (학과명), introduction (한 줄 소개), reason (추천 이유), curriculum (주요 과목 3~4개 배열), career (진출 분야 3~4개 배열).
            """
            ...
            with st.spinner("AI가 당신의 진로를 분석하고 있습니다... 🧠"):
                ...
                # 결과가 리스트가 아니라면 리스트로 감싸기 (혹시 모를 오류 방지)
                if not isinstance(result_data, list):
                    # Gemini가 가끔 {"recommendations": [...]} 형태로 줄 때가 있음
                    result_data = result_data.get("recommendations", result_data)
            ...
            for idx, item in enumerate(result_data):
                # 텍스트 파일 내용 추가
                txt_content += f"====================================\n"
                txt_content += f"NO.{idx + 1}  {item.get('majorName')}\n"
                txt_content += f"====================================\n"
                txt_content += f"1. 학과 소개: {item.get('introduction')}\n"
                txt_content += f"2. 추천 이유: {item.get('reason')}\n"
                txt_content += f"3. 주요 과목: {', '.join(item.get('curriculum', []))}\n"
                txt_content += f"4. 졸업 후 진로: {', '.join(item.get('career', []))}\n\n"

                # 화면에 카드 형태로 표시
                curriculum_tags = "".join([f"<span class='tag'>{c}</span>" for c in item.get('curriculum', [])])
                career_text = ", ".join(item.get('career', []))
                ...
                <div class="card">
                    <div style="display:flex; align-items:center; margin-bottom:10px;">
                        ...
                        <h3 style="margin:0; color:#1e293b;">{item.get('majorName')}</h3>
                    </div>
                    <p style="color:#4b5563; font-style:italic; margin-bottom:15px;">"{item.get('introduction')}"</p>
                    ...
                    <div style="background:white; padding:15px; border-radius:5px; border:1px solid #e2e8f0; margin-bottom:10px;">
                        ...
                        <p style="margin-top:5px; color:#334155;">{item.get('reason')}</p>
                    </div>
            ...
            st.download_button(
                label="📥 결과 저장하기 (.txt)",
                data=txt_content,
                file_name=f"진로추천결과_{int(time.time())}.txt",
                mime="text/plain"
            )
